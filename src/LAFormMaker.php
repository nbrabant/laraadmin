...
class LAFormMaker
{
	...
	/**
	 * Process the input route fields
	 */
	public static function input_route($module, $field_name, $label, $default_val = null, $field_type = 'text', $params = [])
	{
		if (!empty($params) || !isset($params['class'])) {
		...
		// Override the edit value
		if ($module->routable && isset($module->row)) {
			$row = $module->row->routable;
			if ($row !== false) {
				$default_val = $row->{str_replace('route.', '', $field_name)};
			}
		}

		$out = '<div class="form-group">';
		$out .= '<label for="'.$field_name.'">'.$label . (!is_null($required) && $required ? '*' : '') . ' :</label>';
