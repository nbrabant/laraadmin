...
class CodeGenerator
{
    ...
    public static function createModel($config, $comm = null)
    {
        ...
        $md = str_replace("__db_table_name__", $config->dbTableName, $md);
        $md = str_replace("__routable_dependencie__", ($config->routable ? 'use Dwij\Laraadmin\Helpers\RoutableTraitHelper;' : ''), $md);
        $md = str_replace("__routable_usage__", ($config->routable ? 'use RoutableTraitHelper;' : ''), $md);
    ...
    public static function createViews($config, $comm = null)
    {
		...
		$routableFields = '';
        if ($config->routable) {
            $routableFields .= "\t\t\t\t\t<div class=\"header\">\n";
            $routableFields .= "\t\t\t\t\t\t<h4 class=\"title\">Route & SEO</h4>\n";
            $routableFields .= "\t\t\t\t\t\t<p class=\"category\">Route & optimisation pour les moteurs de recherche</p>\n";
            $routableFields .= "\t\t\t\t\t</div>\n";
            $routableFields .= "\t\t\t\t\t@input_route($"."module, 'route.name',                'Route',            null, 'text',       ['required' => true])\n";
            $routableFields .= "\t\t\t\t\t@input_route($"."module, 'route.meta_title',          'Meta titre',       null, 'text')\n";
            $routableFields .= "\t\t\t\t\t@input_route($"."module, 'route.meta_description',    'Meta description', null, 'text')\n";
            $routableFields .= "\t\t\t\t\t@input_route($"."module, 'route.meta_keywords',       'Meta keywords',    null, 'text')\n";
            $routableFields .= "\t\t\t\t\t@input_route($"."module, 'route.robots_index',        'Robot index',      null, 'checkbox',   ['required' => true])\n";
            $routableFields .= "\t\t\t\t\t@input_route($"."module, 'route.robots_follow',       'Robot follow',     null, 'checkbox',   ['required' => true])\n";
        }
        $md = str_replace("__routable_fields__", $routableFields, $md);
